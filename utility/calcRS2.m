...
modsalone = findprop(modulestruct,'isalone');
modstogether = findprop(modulestruct,'isalone',false);
RSmods = findprop(modulestruct,'isResourceSensor');
...
if ploton
    ...
    for k = 1:length(RSnames)
        ...
        yvec{k} = [yalone/yalone; yperturb(:)/yalone];
        ystdvec{k} = [yalonestd/yalone; yperturbstd(:)/yalone];
    ...
    ebh1 = errorbar(xvec,Qvec,Qvecstd,'.k','linewidth',1.5);
    xticklabels(RSnames);
