function [modsoutalone,RSmodsout] = calcQS2(modstruct,RSmodules,ploton,fighy,fighyRS,fighQ,fighS)
...
if ploton
    ...
    for n = 1:length(RSmodnames)
        ...
        for k = 1:length(RSperturb)
            if isfield(modsoutalone,RSperturb{k})
                uRS{k} = modsoutalone.(RSperturb{k}).u;
            else; uRS{k} = 0;
            end
        end
        
        %plot
        figure(fighyRS); subplot(2,1,n);
        plotRSycurves(uRS,RSyperturb,RSyperturbstd,RSyalone,RSyalonestd);
        xlabel('Input, u [nM]'); ylabel('Resource Sensor y, AU')
        legend(['alone'],'Location','best')
        title(RSmodnames{n}); ylim([0, Inf])
    end
    
    %plot module y
    constmodnames = fieldnames(modsoutalone);
    [u,yvec,ystdvec,Qvec,Qvecstd,Svec,Svecstd,Svec2,Svec2std,Svec3,Svec3std] = ...
        deal(cell(length(constmodnames),1));
    for m = 1:length(constmodnames)
        mod2 = modsoutalone.(constmodnames{m});
        u{m} = mod2.u;
        yalone = mod2.y;
        yalonestd = mod2.ystd;
        yperturb = mod2.perturby;
...
S2 = ((modtogeth - modalone)./modalone).*...
    ((RSalone + QRS*(RSalone - RStogeth))./(QRS*RSalone*(QRS + 1)));
%module sensitivity = Fhat; from CDC paper
...
function [yvec,ystdvec,RSyvec,RSystdvec,Qvec,Qvecstd,Svec,Svecstd] = ...
    constplots(constmods,RSmodsout)
%constitutive modules
%plot mean and std of RS sensor outputs y and Q
constmodnames = fieldnames(constmods);
...
for m = 1:length(constmodnames)
    ...
    Svecstd{m} = mod.Sstd;
    Svec2{m} = mod.S2;
    Svec2std{m} = mod.S2std;
    Svec3{m} = mod.S3;
    Svec3std{m} = mod.S3std;
end
