function [outstruct,basalstruct] = subtractbasal4(cellstruct,nonestruct,ODfld,subfields)
%[newstruct,basalstruct] = subtractbasal4(cellstct,nonefld,mediafld,ODeps,FPeps)
...
for ii = 1:length(cellnames)
    ...
    data = cellstruct.(cellnames{ii});
    %loop across each cell type
    dprops = fieldnames(data);
    ...
    for jj = 1:length(dprops)
        ...
        if any(strcmpi(dprops{jj},subfields))
            if strcmp(dprops{jj},ODfld)
                ...
                outobj.(dprops{jj}) = data.(dprops{jj}) - basalstruct.(dprops{jj});
            else
                %if FP, loop across each data property and subtract off basal
                %level, dependent on OD
                FPODfun = basalstruct.(dprops{jj});
                outobj.(dprops{jj}) = data.(dprops{jj}) - FPODfun(ODdata);
            end
        else
            %copy remaining data
            outobj.(dprops{jj}) = data.(dprops{jj});
        end
    end
