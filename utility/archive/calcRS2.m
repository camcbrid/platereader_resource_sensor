function RSmodules = calcRS2(modulestruct,ploton,fighQ,fighy)
%RSmodules = calcRSall2(modulestruct,ploton,figh)
...
%NOTE: I eleminated monte-carlo & least squares formulation functionality
%in this version. Use calcRSall.m for that (uses different input/output
%data formatting though).
...
for ii = 1:length(RSmodstogeth)
    ...
    alone1.S = -1/(1 + Q1 + Q2);
    alone1.Sstd = sqrt(Q1std^2/(1+Q1+Q2)^2 + Q2std^2/(1+Q1+Q2)^2);
    alone2.S = -1/(1 + Q1 + Q2);
    alone2.Sstd = sqrt(Q1std^2/(1+Q1+Q2)^2 + Q2std^2/(1+Q1+Q2)^2);
    alone1.S2 = -(1 + Q1)/(1 + Q1 + Q2).^2;
    alone1.S2std = sqrt(((1+Q1-Q2)/(1+Q1+Q2)^3)*Q1std^2 + (2*(1+Q1)/(1+Q1+Q2)^3)*Q2std^2);
    alone2.S2 = -(1 + Q2)/(1 + Q1 + Q2).^2;
    alone2.S2std = sqrt(((1+Q2-Q1)/(1+Q1+Q2)^3)*Q2std^2 + (2*(1+Q2)/(1+Q1+Q2)^3)*Q1std^2);
    alone1.S3 = 1;
    alone1.S3std = 0;
    alone2.S3 = 1;
    alone2.S3std = 0;
    ...
    alone2.S4std = 0;
    alone1.S5 = alone1.S;
    alone1.S5std = alone1.Sstd;
    alone2.S5 = alone2.S;
    alone2.S5std = alone2.Sstd;
    alone1.S6 = alone1.S2;
    alone1.S6std = alone1.S2std;
    alone2.S6 = alone2.S2;
    alone2.S6std = alone2.S2std;
    alone1.S7 = 1;
    alone1.S7std = 0;
    alone2.S7 = 1;
    alone2.S7std = 0;
