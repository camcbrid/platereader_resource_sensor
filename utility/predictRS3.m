function outstruct = predictRS3(modstruct,modulesout,ploton,figh)
%predict outputs HELP
...
function [pred,predstd] = applyRSpred(y,S,Q1,Q2,ystd,Q1std,Q2std,Sstd)

%use resource demand to predict circuit expression
pred = y*S*(1+Q1)./(1+Q1+Q2);

%calculate standard error on predicted output
dfdy = S*(1+Q1)./(1+Q1+Q2);
dfdS = y*(1+Q1)./(1+Q1+Q2);
dfdQ1 = y*S*Q2./(1+Q1+Q2).^2;
dfdQ2 = -y*S*(1+Q1)./(1+Q1+Q2).^2;
predstd = sqrt(dfdy.^2*ystd.^2 + dfdS.^2*Sstd.^2 + dfdQ1.^2*Q1std.^2 + dfdQ2.^2*Q2std.^2);
